...
export function extractPeaks(positivePath: [number, number][], x: number) {
  let peaks = [];
  let peakPoints = [];
  let findFlag = false;
...
export function cutOffAllTracks(allTracks: any, traceCount: number) {
...
export function tracking(allPeaks: any, startTrackNumber: number) {
  const allTracks = [];
  for (let i = 0; i < allPeaks[startTrackNumber].length; i++) {
    ...
    for (let j = startTrackNumber + 1; j < allPeaks.length; j++) {
      ...
      for (let s = 0; s < allPeaks[j].length; s++) {
        let offset = Math.abs(allPeaks[j][s].mid - track[track.length - 1].mid);
        if (offset < MaxOffSet) {
          MaxOffSet = offset;
  ...
  function cutoff(allTracks: any[]) {
    for (let i = 0; i < allTracks.length - 1; i++) {
      for (let j = 0; j < allTracks[i].length; j++) {
        ...
        let curValueDiff = Math.abs(curTrack[j].value - curTrack[j - 1].value);
